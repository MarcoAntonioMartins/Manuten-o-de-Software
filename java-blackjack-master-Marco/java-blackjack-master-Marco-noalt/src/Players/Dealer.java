...
public class Dealer extends BlackjackPlayer {
    ...
    public boolean acceptBetFrom(Player player, double bet) {
        ...
        if (player.betPlaced()) {
            say("Obrigado, você selecionou a aposta de $" + player.getBet() + ". Gostaria de aumentar?");
        } else {
            say("Por favor selecione sua aposta.");
        }
    ...
    public void hit(Player player) {
        say(player.getName() + " Bateu.");
        player.hand.add(deck.deal());

        playerCanDouble = false;

        if (player.hand.isBust()) {
            say(player.getName() + " Estourou. Perdeu $" + player.getBet());
            player.loses();
            gameOver = true;
        }
    }
    ...
    public void playDouble(Player player) {
        if (player.doubleBet() && playerCanDouble) {
            player.hand.add(deck.deal());
            say(player.getName() + " Jogador Dobrou.");
            ...
            say(player.getName() + ", não pode dobrar. Sem Dinheiro suficiente.");
        }
    }
    ...
    public void stand(Player player) {
        say(player.getName() + " mantem. " + this.getName() + " Tuno.");
        go(player);
    }
    ...
    private void go(Player player) {
        ...
        if (!hand.hasBlackjack()) {
            while (hand.getTotal() < DEALER_STANDS_ON) {
                hand.add(deck.deal());
                say(this.getName() + " beteu.");
            }

            if (hand.isBust()) {
                say(this.getName() + " tá estourado");
            } else {
                say(this.getName() + " mantem os " + hand.getTotal());
            }
        } else {
            say(this.getName() + " TEM BLACKJACK!!!");
        }
        ...
        if (hand.hasBlackjack() && player.hand.hasBlackjack()) {
            ...
            say(player.getName() + " ganhou com Blackjack $" + winnings);
            player.wins(player.getBet() + winnings);
        } else if (hand.hasBlackjack()) {
            say("Dealer tem Blackjack. " + player.getName() + " perdeu $" + player.getBet());
            player.loses();
        } else if (hand.isBust()) {
            say("Dealer estourou. " + player.getName() + " ganhou $" + player.getBet());
            player.wins(player.getBet() * 2);
        } else if (player.hand.getTotal() == hand.getTotal()) {
            say("Push");
            player.clearBet();
        } else if (player.hand.getTotal() < hand.getTotal()) {
            say(player.getName() + " perdeu $" + player.getBet());
            player.loses();
        } else if (player.hand.getTotal() > hand.getTotal()) {
            say(player.getName() + " ganhou $" + player.getBet());
            player.wins(player.getBet() * 2);
        }
